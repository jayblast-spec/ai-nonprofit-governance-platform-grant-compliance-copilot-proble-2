...
//
// Rules enforced:
//   - No markdown (.md) files inside app/ or public/ â€” docs live under /docs only.
...
for (const dir of ['app', 'public']) {
  for (const file of walk(path.join(root, dir))) {
    if (file.endsWith('.md')) {
      errors.push(`Markdown file found in /${dir}: ${path.relative(root, file)} â€” docs must live under /docs, not /${dir}.`);
    }
  }
}
...
for (const file of walk(path.join(root, 'app'))) {
  ...
  if (MARKDOWN_LEAK_PATTERN.test(content)) {
    errors.push(`${path.relative(root, file)} appears to contain raw markdown/README content â€” marketing copy must come from /content/marketing, not docs.`);
  }
}
...
if (!fs.existsSync(heroPath)) {
  errors.push('content/marketing/en/hero.json is missing â€” landing copy must be segmented by audience.');
} else {
...
if (!fs.existsSync(trustPath)) {
  errors.push('content/marketing/en/trust.json is missing â€” trust copy must be segmented by audience.');
} else {
